    ...
    public function register(Request $request, UserPasswordEncoderInterface $passwordEncoder, GuardAuthenticatorHandler $guardHandler, CustomAuthenticator $authenticator): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($form->get('plainPassword')->getData() == $form->get('passwordRepeat')->getData()) {
                ...
                $user->setLastLogin(DateTime::createFromFormat('Y-m-d H:i:s', date('Y-m-d H:i:s')));
                ...
                $this->emailVerifier->sendEmailConfirmation(
                    ...
                    $user,
                    (new TemplatedEmail())
                        ->from(new Address('home@martinruzek.cz', 'Ruzkovi Mail Bot'))
                        ->to($user->getEmail())
